...
namespace Vs.Category
{
    public partial class frmEditCHE_DO_NGHI : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void frmEditCHE_DO_NGHI_Load(object sender, EventArgs e)
        {
            ...
            {
                string strSQL = "SELECT MAX(STT) FROM dbo.CHE_DO_NGHI";
        ...
        private bool bKiemTrung()
        {
            try
            {
                ...
                iKiem = Convert.ToInt16(SqlHelper.ExecuteScalar(Commons.IConnections.CNStr, "spCheckData", "ID_CHE_DO",
                    (AddEdit ? "-1" : Id.ToString()), "CHE_DO_NGHI", "TEN_CHE_DO", TEN_CHE_DOTextEdit.Text.ToString(),
                    "", "", "", ""));
                if (iKiem > 0)
                {
                ...
                {
                    iKiem = Convert.ToInt16(SqlHelper.ExecuteScalar(Commons.IConnections.CNStr, "spCheckData", "ID_CHE_DO",
                        (AddEdit ? "-1" : Id.ToString()), "CHE_DO_NGHI", "TEN_CHE_DO_A", TEN_CHE_DO_ATextEdit.Text.ToString(),
                        "", "", "", ""));
                    if (iKiem > 0)
                    {
                        XtraMessageBox.Show(Commons.Modules.ObjLanguages.GetLanguage("frmMessage", "msg_TenTrung"), Commons.Modules.ObjLanguages.GetLanguage("msgThongBao", "msg_Caption"), MessageBoxButtons.OK, MessageBoxIcon.Information);
                ...
                {
                    iKiem = Convert.ToInt16(SqlHelper.ExecuteScalar(Commons.IConnections.CNStr, "spCheckData", "ID_CHE_DO",
                        (AddEdit ? "-1" : Id.ToString()), "CHE_DO_NGHI", "TEN_CHE_DO_H", TEN_CHE_DO_HTextEdit.Text.ToString(),
                        "", "", "", ""));
                    if (iKiem > 0)
                    {
                        XtraMessageBox.Show(Commons.Modules.ObjLanguages.GetLanguage("frmMessage", "msg_TenTrung"), Commons.Modules.ObjLanguages.GetLanguage("msgThongBao", "msg_Caption"), MessageBoxButtons.OK, MessageBoxIcon.Information);
