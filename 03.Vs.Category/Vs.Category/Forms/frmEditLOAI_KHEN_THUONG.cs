...
{
    public partial class frmEditLOAI_KHEN_THUONG : DevExpress.XtraEditors.XtraForm
    {
        Int64 Id = 0;
        Boolean AddEdit = true;  // true la add false la edit

        public frmEditLOAI_KHEN_THUONG(Int64 iId, Boolean bAddEdit)
        {
            InitializeComponent();
            Id = iId;
            AddEdit = bAddEdit;
        ...
        private void frmEditLOAI_KHEN_THUONG_Resize(object sender, EventArgs e) => dataLayoutControl1.Refresh();

        private void LoadText()
        {
            try
            {
                string sSql = "SELECT ID_LOAI_KT, TEN_LOAI_KT, TEN_LOAI_KT_A, TEN_LOAI_KT_H, STT " +
                    "FROM LOAI_KHEN_THUONG WHERE ID_LOAI_KT = " + Id.ToString();
                DataTable dtTmp = new DataTable();
                dtTmp.Load(SqlHelper.ExecuteReader(Commons.IConnections.CNStr, CommandType.Text, sSql));
                TEN_LOAI_KTTextEdit.EditValue = dtTmp.Rows[0]["TEN_LOAI_KT"].ToString();
                TEN_LOAI_KT_ATextEdit.EditValue = dtTmp.Rows[0]["TEN_LOAI_KT_A"].ToString();
                TEN_LOAI_KT_HTextEdit.EditValue = dtTmp.Rows[0]["TEN_LOAI_KT_H"].ToString();
        ...
        private void LoadTextNull()
        {
            try
            {
                TEN_LOAI_KTTextEdit.EditValue = String.Empty;
                TEN_LOAI_KT_ATextEdit.EditValue = String.Empty;
                TEN_LOAI_KT_HTextEdit.EditValue = String.Empty;
                txtSTT.EditValue = 1;
                TEN_LOAI_KTTextEdit.Focus();
            }
            catch { }
        }
                    ...
                    case "luu":
                        {
                            ...
                            Commons.Modules.sId = SqlHelper.ExecuteScalar(Commons.IConnections.CNStr, "spUpdateLOAI_KHEN_THUONG", (AddEdit ? -1 : Id),
                                TEN_LOAI_KTTextEdit.EditValue, TEN_LOAI_KT_ATextEdit.EditValue, TEN_LOAI_KT_HTextEdit.EditValue, (txtSTT.EditValue == "") ? txtSTT.EditValue = null : txtSTT.EditValue).ToString();
                            if (AddEdit)
                            {
