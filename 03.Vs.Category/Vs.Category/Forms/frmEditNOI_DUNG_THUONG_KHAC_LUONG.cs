...
namespace Vs.Category
{
    public partial class frmEditNOI_DUNG_THUONG_KHAC_LUONG : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void frmEditNOI_DUNG_THUONG_KHAC_LUONG_Load(object sender, EventArgs e)
        {
            ...
            {
                string strSQL = "SELECT MAX(STT) FROM dbo.NOI_DUNG_THUONG_KHAC_LUONG";
        ...
        private bool bKiemTrung()
        {
            try
            {
                ...
                iKiem = Convert.ToInt16(SqlHelper.ExecuteScalar(Commons.IConnections.CNStr, "spCheckData", "ID_NDTKL",
                    (AddEdit ? "-1" : Id.ToString()), "NOI_DUNG_THUONG_KHAC_LUONG", "TEN_THUONG", TEN_TKLTextEdit.Text.ToString(),
                    "", "", "", ""));
                if (iKiem > 0)
                {
                ...
                {
                    iKiem = Convert.ToInt16(SqlHelper.ExecuteScalar(Commons.IConnections.CNStr, "spCheckData", "ID_NDTKL",
                        (AddEdit ? "-1" : Id.ToString()), "NOI_DUNG_THUONG_KHAC_LUONG", "TEN_THUONG_A", TEN_TKL_ATextEdit.Text.ToString(),
                        "", "", "", ""));
                    if (iKiem > 0)
                    {
                        XtraMessageBox.Show(Commons.Modules.ObjLanguages.GetLanguage("frmMessage", "msg_TenTrung"), Commons.Modules.ObjLanguages.GetLanguage("msgThongBao", "msg_Caption"), MessageBoxButtons.OK, MessageBoxIcon.Information);
                ...
                {
                    iKiem = Convert.ToInt16(SqlHelper.ExecuteScalar(Commons.IConnections.CNStr, "spCheckData", "ID_NDTKL",
                        (AddEdit ? "-1" : Id.ToString()), "NOI_DUNG_THUONG_KHAC_LUONG", "TEN_THUONG_H", TEN_TKL_HTextEdit.Text.ToString(),
                        "", "", "", ""));
                    if (iKiem > 0)
                    {
                        XtraMessageBox.Show(Commons.Modules.ObjLanguages.GetLanguage("frmMessage", "msg_TenTrung"), Commons.Modules.ObjLanguages.GetLanguage("msgThongBao", "msg_Caption"), MessageBoxButtons.OK, MessageBoxIcon.Information);
