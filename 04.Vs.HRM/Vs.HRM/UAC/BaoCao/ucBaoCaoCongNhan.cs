    ...
    public partial class ucBaoCaoCongNhan : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        private void windowsUIButton_ButtonClick(object sender, ButtonEventArgs e)
        {
            ...
            switch (btn.Tag.ToString())
            {
                ...
                case "Print":
                    {
                        if (rdoChonBC.SelectedIndex == 4)
                        {
                            InExcelBCDanhSachCongNhanDoiHoSo();
                            return;
                        }
                        switch (Commons.Modules.KyHieuDV)
                        {
        ...
        private void DanhSachNhanVien_Chung()
        {
            try
            {
                ...
                try
                {
                    ...
                    int lastColumn = 0;
                    lastColumn = dtBCThang.Columns.Count - 5;
                    int row_dl = 8;

                    TaoTTChung_TheoDV(oSheet, 1, 3, 1, 7, 0, 0);
                    string sTieuDeBC = "";
                    if (rdoChonBC.SelectedIndex == 2)
                    {
                        sTieuDeBC = "DANH SÁCH CÁN BỘ CÔNG NHÂN VIÊN MỚI VÀO LÀM VIỆC";
                    }
                    else if (rdoChonBC.SelectedIndex == 3)
                    {
                        sTieuDeBC = "DANH SÁCH CÁN BỘ CÔNG NHÂN VIÊN NGHỈ VIỆC";
                    }
                    else
                    {
                        sTieuDeBC = "DANH SÁCH NHÂN VIÊN";
                    }
                    ...
                    row2_TieuDe_BaoCao.Cells.HorizontalAlignment = Excel.XlHAlign.xlHAlignCenter;
                    row2_TieuDe_BaoCao.Cells.VerticalAlignment = Excel.XlVAlign.xlVAlignCenter;
                    row2_TieuDe_BaoCao.Value2 = Commons.Modules.KyHieuDV == "NB" ? sTieuDeBC : "DANH SÁCH NHÂN VIÊN";
                    ...
                    if (rdoChonBC.SelectedIndex == 2 || rdoChonBC.SelectedIndex == 3)
                    {
                        ...
                        row2_TieuDe_BaoCao.Cells.HorizontalAlignment = Excel.XlHAlign.xlHAlignCenter;
                        row2_TieuDe_BaoCao.Cells.VerticalAlignment = Excel.XlVAlign.xlVAlignCenter;
                        row2_TieuDe_BaoCao.Value2 = "Từ ngày " + datTuNgay.Text + " đến ngày " + datDenNgay.Text;
                    }

                    Excel.Range row5_TieuDe_Format = oSheet.Range[oSheet.Cells[row_dl, 1], oSheet.Cells[row_dl, lastColumn]];
                    row5_TieuDe_Format.Font.Size = fontSizeTieuDe;
                    row5_TieuDe_Format.Font.Name = fontName;
                    row5_TieuDe_Format.Font.Bold = true;
                    row5_TieuDe_Format.WrapText = true;
                    ...
                    oSheet.Cells[row_dl, 1].ColumnWidth = 3;
                    ...
                    if (chkGroup.Checked == false)
                    {
                        ...
                        keepRowCnt = rowCnt + 8;
                        oSheet.Range[oSheet.Cells[row_dl, 1], oSheet.Cells[keepRowCnt, lastColumn]].Value2 = rowData;

                        formatRange = oSheet.Range[oSheet.Cells[9, 1], oSheet.Cells[keepRowCnt.ToString(), 1]];
                        formatRange.NumberFormat = "#,##0;(#,##0);;";
                        formatRange.Cells.HorizontalAlignment = Excel.XlHAlign.xlHAlignCenter;
                        formatRange.Cells.VerticalAlignment = Excel.XlVAlign.xlVAlignCenter;
                        ...
                        for (col = 1; col < lastColumn - 1; col++)
                        {
                            formatRange = oSheet.Range[oSheet.Cells[row_dl, col + 1], oSheet.Cells[(rowCnt + 4).ToString(), col + 1]];
                            if (dt.Rows[col - 1]["DINH_DANG"].ToString() == "Num")
                            {
                                formatRange.NumberFormat = "#,##0;(#,##0); ; ";
                                try { formatRange.TextToColumns(Type.Missing, Excel.XlTextParsingType.xlDelimited, Excel.XlTextQualifier.xlTextQualifierDoubleQuote); } catch (Exception ex) { }
                    ...
                    else
                    {
                        ...
                        for (int j = 0; j < TEN_TO.Count(); j++)
                        {
                            ...
                            formatRange = oSheet.Range[oSheet.Cells[(rowBD + 1), 1], oSheet.Cells[(rowCnt + 1), 1]];
                            formatRange.NumberFormat = "#,##0;(#,##0); ; ";
                            try { formatRange.TextToColumns(Type.Missing, Excel.XlTextParsingType.xlDelimited, Excel.XlTextQualifier.xlTextQualifierDoubleQuote); } catch (Exception ex) { }

                            for (col = 1; col <= lastColumn - 1; col++)
                            {
                                formatRange = oSheet.Range[oSheet.Cells[(rowBD + 1), col + 1], oSheet.Cells[(rowCnt + 1), col + 1]];
                                if (dt.Rows[col - 1]["DINH_DANG"].ToString() == "Num")
                                {
                                    formatRange.NumberFormat = "#,##0;(#,##0); ; ";
                                    try { formatRange.TextToColumns(Type.Missing, Excel.XlTextParsingType.xlDelimited, Excel.XlTextQualifier.xlTextQualifierDoubleQuote); } catch (Exception ex) { }
                    ...
                    rowCnt = keepRowCnt;
                    formatRange = oSheet.Range[oSheet.Cells[row_dl, 1], oSheet.Cells[(rowCnt + 1), lastColumn]];
                    formatRange.Font.Name = fontName;
                    formatRange.Font.Size = fontSizeNoiDung;
                    formatRange.Cells.VerticalAlignment = Excel.XlVAlign.xlVAlignCenter;
                    BorderAround(oSheet.Range[oSheet.Cells[row_dl - 1, 1], oSheet.Cells[(rowCnt + 1), lastColumn]]);

                    if (dtBCThang.Rows.Count == 0)
                    {
                        Excel.Range myRange = oSheet.Range[oSheet.Cells[row_dl - 1, 1], oSheet.Cells[row_dl - 1, lastColumn]];
                        myRange.AutoFilter("1", "<>", Excel.XlAutoFilterOperator.xlOr, "", true);
                    }
                    else
                    {
                        Excel.Range myRange = oSheet.Range[oSheet.Cells[row_dl - 1, 1], oSheet.Cells[(rowCnt - 1), lastColumn]];
                        myRange.AutoFilter("1", "<>", Excel.XlAutoFilterOperator.xlOr, "", true);
                    }
                    ...
                    #region Người lập biểu
                    oSheet.get_Range("C" + (rowCnt) + "", "C" + (rowCnt).ToString()).Value2 = "Người lập biểu";
                    oSheet.get_Range("C" + (rowCnt) + "", "C" + (rowCnt).ToString()).Font.Size = 12;
                    oSheet.get_Range("C" + (rowCnt) + "", "C" + (rowCnt).ToString()).Font.Bold = true;
                    oSheet.get_Range("C" + (rowCnt) + "", "C" + (rowCnt).ToString()).Font.Name = fontName;
                    oSheet.get_Range("C" + (rowCnt) + "", "C" + (rowCnt).ToString()).Cells.HorizontalAlignment = Excel.XlHAlign.xlHAlignCenter;
                    oSheet.get_Range("C" + (rowCnt) + "", "C" + (rowCnt).ToString()).Cells.VerticalAlignment = Excel.XlVAlign.xlVAlignCenter;

                    oSheet.get_Range("G" + (rowCnt) + "", "G" + (rowCnt).ToString()).Value2 = "Phòng HCNS";
                    oSheet.get_Range("G" + (rowCnt) + "", "G" + (rowCnt).ToString()).Font.Size = 12;
                    oSheet.get_Range("G" + (rowCnt) + "", "G" + (rowCnt).ToString()).Font.Bold = true;
                    oSheet.get_Range("G" + (rowCnt) + "", "G" + (rowCnt).ToString()).Font.Name = fontName;
                    oSheet.get_Range("G" + (rowCnt) + "", "G" + (rowCnt).ToString()).Cells.HorizontalAlignment = Excel.XlHAlign.xlHAlignCenter;
                    oSheet.get_Range("G" + (rowCnt) + "", "G" + (rowCnt).ToString()).Cells.VerticalAlignment = Excel.XlVAlign.xlVAlignCenter;

                    string sTenTinh = Convert.ToString(SqlHelper.ExecuteScalar(Commons.IConnections.CNStr, CommandType.Text, "SELECT TOP 1 ISNULL(TEN_TINH,'') FROM dbo.DON_VI"));
                    oSheet.get_Range(CharacterIncrement(lastColumn - 2) + (rowCnt) + "", CharacterIncrement(lastColumn - 2) + (rowCnt).ToString()).Value2 = sTenTinh + ", ngày " + NgayIn.DateTime.Day + " tháng " + NgayIn.DateTime.Month + "  năm " + NgayIn.DateTime.Year;
                    oSheet.get_Range(CharacterIncrement(lastColumn - 2) + (rowCnt) + "", CharacterIncrement(lastColumn - 2) + (rowCnt).ToString()).Font.Size = 12;
                    oSheet.get_Range(CharacterIncrement(lastColumn - 2) + (rowCnt) + "", CharacterIncrement(lastColumn - 2) + (rowCnt).ToString()).Font.Italic = true;
                    oSheet.get_Range(CharacterIncrement(lastColumn - 2) + (rowCnt) + "", CharacterIncrement(lastColumn - 2) + (rowCnt).ToString()).Font.Name = fontName;
                    oSheet.get_Range(CharacterIncrement(lastColumn - 2) + (rowCnt) + "", CharacterIncrement(lastColumn - 2) + (rowCnt).ToString()).Cells.HorizontalAlignment = Excel.XlHAlign.xlHAlignCenter;
                    oSheet.get_Range(CharacterIncrement(lastColumn - 2) + (rowCnt) + "", CharacterIncrement(lastColumn - 2) + (rowCnt).ToString()).Cells.VerticalAlignment = Excel.XlVAlign.xlVAlignCenter;

                    rowCnt++;
                    oSheet.get_Range(CharacterIncrement(lastColumn - 2) + (rowCnt) + "", CharacterIncrement(lastColumn - 2) + (rowCnt).ToString()).Value2 = "BAN GIÁM ĐỐC";
                    oSheet.get_Range(CharacterIncrement(lastColumn - 2) + (rowCnt) + "", CharacterIncrement(lastColumn - 2) + (rowCnt).ToString()).Font.Size = 12;
                    oSheet.get_Range(CharacterIncrement(lastColumn - 2) + (rowCnt) + "", CharacterIncrement(lastColumn - 2) + (rowCnt).ToString()).Font.Bold = true;
                    oSheet.get_Range(CharacterIncrement(lastColumn - 2) + (rowCnt) + "", CharacterIncrement(lastColumn - 2) + (rowCnt).ToString()).Font.Name = fontName;
                    oSheet.get_Range(CharacterIncrement(lastColumn - 2) + (rowCnt) + "", CharacterIncrement(lastColumn - 2) + (rowCnt).ToString()).Cells.HorizontalAlignment = Excel.XlHAlign.xlHAlignCenter;
                    oSheet.get_Range(CharacterIncrement(lastColumn - 2) + (rowCnt) + "", CharacterIncrement(lastColumn - 2) + (rowCnt).ToString()).Cells.VerticalAlignment = Excel.XlVAlign.xlVAlignCenter;
                    #endregion
                    ...
                    oSheet.PageSetup.TopMargin = oXL.InchesToPoints(0.25);
                    oSheet.PageSetup.BottomMargin = oXL.InchesToPoints(0.25);
        ...
        public void CreateHeaderTable(ref Excel.Worksheet oSheet, string fontName = "Times New Roman", int fontSizeNoiDung = 11)
        {
            ...
            FormatTitleTable(ref row5_Header_Table_Nguyen_Quan, fontName, fontSizeNoiDung, 75, height_Single, Color.FromArgb(255, 255, 255), true, "Địa chỉ khai sinh");
            ...
            Range row5_Header_Table_AB6 = oSheet.get_Range("AB10", "AB10"); // AB6
            ...
            Range row6_Header_Table_AC6 = oSheet.get_Range("AC10", "AC10"); // AC6
            ...
            Range row7_Header_Table_AD6 = oSheet.get_Range("AD10", "AD10"); // AD6
            ...
            FormatTitleTable(ref row6_Header_Table_Ngay_Ky_HDLD, fontName, fontSizeNoiDung, 15, height_Single, Color.FromArgb(255, 255, 255), false, "Ngày bắt đầu HĐ");
            ...
            Range row5_Header_Table_SoCMNDCu = oSheet.get_Range("BN9", "BN10"); // 
            FormatTitleTable(ref row5_Header_Table_SoCMNDCu, fontName, fontSizeNoiDung, 15, height_Single, Color.FromArgb(255, 255, 255), true, "Số CMND cũ");
            Range row5_Header_Table_BN11 = oSheet.get_Range("BN11"); // 
            FormatTitleTable(ref row5_Header_Table_BN11, fontName, fontSizeNoiDung, 15, height_Single, Color.FromArgb(255, 255, 255), false, "66");
        }
        ...
        private void HeaderReport(ref Excel.Worksheet oSheet, string fontName = "Times New Roman", int fontSizeNoiDung = 11, string lastColumn = "", int fontSizeTieuDe = 11, int Tcot = 0)
        {
            ...
            Range row3_TieuDe_BaoCao = oSheet.get_Range("A5", "A5"); // = A6 - BL6
            FormatTieuDeBaoCao(ref row3_TieuDe_BaoCao, false, true, 18, fontName, "@", XlHAlign.xlHAlignLeft, XlVAlign.xlVAlignCenter, "DANH SÁCH CÁN BỘ CÔNG NHÂN VIÊN NHÀ MÁY");
            ...
            Range row4_TieuDe_BaoCao = oSheet.get_Range("A6", "A6"); // = A7 - BL7
            FormatTieuDeBaoCao(ref row4_TieuDe_BaoCao, true, true, 11, fontName, "@", XlHAlign.xlHAlignLeft, XlVAlign.xlVAlignCenter, "Ngày " + Ngay + " Tháng " + Thang + " Năm " + Nam);
        ...
        private void DanhSachCanBoCongNhanVien()
        {
            ...
            try
            {
                ...
                cmd.Parameters.Add("@LOAI", SqlDbType.Int).Value = rdoChonBC.SelectedIndex;
                cmd.Parameters.Add("@TU_NGAY", SqlDbType.Date).Value = datTuNgay.DateTime;
                cmd.Parameters.Add("@DEN_NGAY", SqlDbType.Date).Value = datDenNgay.DateTime;
                cmd.CommandType = CommandType.StoredProcedure;
                System.Data.SqlClient.SqlDataAdapter adp = new System.Data.SqlClient.SqlDataAdapter(cmd);
                adp.Fill(dt);
                dt.TableName = "DA_TA";
                ...
                Excel.Range formatRange17 = oSheet.get_Range("BK12", "BK" + rowCnt.ToString());////QUÂN SỐ
                formatRange17.NumberFormat = "0";
                try { formatRange17.TextToColumns(Type.Missing, Excel.XlTextParsingType.xlDelimited, Excel.XlTextQualifier.xlTextQualifierDoubleQuote); } catch { }

                Excel.Range formatRange16 = oSheet.get_Range("B12", "B" + rowCnt.ToString());////Format colum AW8 of Data table
                formatRange16.NumberFormat = "0";
                try { formatRange16.TextToColumns(Type.Missing, Excel.XlTextParsingType.xlDelimited, Excel.XlTextQualifier.xlTextQualifierDoubleQuote); } catch { }

                Excel.Range formatRange18 = oSheet.get_Range("AA12", "AA" + rowCnt.ToString());////Mã thẻ cũ
                formatRange18.NumberFormat = "0";
                try { formatRange18.TextToColumns(Type.Missing, Excel.XlTextParsingType.xlDelimited, Excel.XlTextQualifier.xlTextQualifierDoubleQuote); } catch { }

                Excel.Range formatRange19 = oSheet.get_Range("W12", "W" + rowCnt.ToString());////Mã thẻ cũ
                formatRange19.NumberFormat = "0";
                try { formatRange19.TextToColumns(Type.Missing, Excel.XlTextParsingType.xlDelimited, Excel.XlTextQualifier.xlTextQualifierDoubleQuote); } catch { }

                Excel.Range formatRange20 = oSheet.get_Range("X12", "X" + rowCnt.ToString());////Mã thẻ cũ
                formatRange20.NumberFormat = "0";
                try { formatRange20.TextToColumns(Type.Missing, Excel.XlTextParsingType.xlDelimited, Excel.XlTextQualifier.xlTextQualifierDoubleQuote); } catch { }

                Excel.Range range = oSheet.Rows[8];
                range.Delete();
                range = oSheet.Rows[7];
                range.Delete();
        ...
        private void rdoChonBC_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (rdoChonBC.SelectedIndex == 2 || rdoChonBC.SelectedIndex == 3 || rdoChonBC.SelectedIndex == 4)
            {
        ...
        private void InExcelBCDanhSachCongNhanDoiHoSo()
        {
            this.Cursor = Cursors.WaitCursor;
            try
            {
                ...
                conn = new System.Data.SqlClient.SqlConnection(Commons.IConnections.CNStr);
                conn.Open();
                System.Data.SqlClient.SqlCommand cmd = new System.Data.SqlClient.SqlCommand("rptDSCongNhanDoiHoSo", conn);
                ...
                var ws1 = pck.Workbook.Worksheets.Add(Commons.Modules.ObjLanguages.GetLanguage(this.Name, "sBCDanhSachCongNhanDoiHoSo"));

                int iDong = 1;

                Commons.Modules.MExcel.MText(ws1, this.Name, "sBCDanhSachCongNhanDoiHoSo", iDong, 1, iDong, dt1.Columns.Count - 1, true, true, 16, OfficeOpenXml.Style.ExcelHorizontalAlignment.Center, OfficeOpenXml.Style.ExcelVerticalAlignment.Center);
                iDong++;
                Commons.Modules.MExcel.MText(ws1, "", Commons.Modules.ObjLanguages.GetLanguage(this.Name, "sTNgay") + " " + datTuNgay.DateTime.ToString("dd/MM/yyyy") + " " + Commons.Modules.ObjLanguages.GetLanguage(this.Name, "sDNgay") + " " + datDenNgay.DateTime.ToString("dd/MM/yyyy"), iDong, 1, iDong, dt1.Columns.Count - 1, true, true, 16, OfficeOpenXml.Style.ExcelHorizontalAlignment.Center, OfficeOpenXml.Style.ExcelVerticalAlignment.Center);
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    Int64 iID_TO = string.IsNullOrEmpty(Convert.ToString(dr["ID_TO"])) ? 0 : Convert.ToInt64(dr["ID_TO"]);
                    try
                    {
                        dtTMP = dt1.AsEnumerable().Where(r => r.Field<Int64>("ID_TO").Equals(iID_TO)).CopyToDataTable();
                    }
                    catch { dtTMP = dt1.Clone(); }

                    if (dtTMP == null || dtTMP.Rows.Count == 0) continue;
                    dtTMP.Columns.Remove("ID_TO");

                    Commons.Modules.MExcel.MText(ws1, "", string.IsNullOrEmpty(Convert.ToString(dr["TEN_TO"])) ? "" : Convert.ToString(dr["TEN_TO"]), iDong, 1, iDong, dtTMP.Columns.Count, true, true, 13, OfficeOpenXml.Style.ExcelHorizontalAlignment.Left, OfficeOpenXml.Style.ExcelVerticalAlignment.Center);

                    iDong++;

                    WidthColumnsName = new List<Object>() { "STT", 10 };
                    WidthColumns.Add(WidthColumnsName);
                    WidthColumnsName = new List<Object>() { "MS_CN", 15 };
                    WidthColumns.Add(WidthColumnsName);
                    WidthColumnsName = new List<Object>() { "HO_TEN", 30 };
                    WidthColumns.Add(WidthColumnsName);
                    WidthColumnsName = new List<Object>() { "NGAY_SINH", 15, "dd/MM/yyyy" };
                    WidthColumns.Add(WidthColumnsName);
                    WidthColumnsName = new List<Object>() { "NGAY_VAO_LAM", 15, "dd/MM/yyyy" };
                    WidthColumns.Add(WidthColumnsName);
                    WidthColumnsName = new List<Object>() { "MS_CN_CU", 15 };
                    WidthColumns.Add(WidthColumnsName);
                    for (int i = 6; i < dtTMP.Columns.Count; i++)
                    {
                        if (dtTMP.Columns[i].ColumnName.Substring(0, 9) == "MS_CN_CU_")
                        {
                            WidthColumnsName = new List<Object>() { dtTMP.Columns[i].ColumnName, 15 };
                            WidthColumns.Add(WidthColumnsName);
                        }
                        else if (dtTMP.Columns[i].ColumnName.Substring(0, 15) == "NGAY_DOI_MS_CN_")
                        {
                            WidthColumnsName = new List<Object>() { dtTMP.Columns[i].ColumnName, 20, "dd/MM/yyyy" };
                            WidthColumns.Add(WidthColumnsName);
                        }
                    }
                    ws1.Cells[iDong, 1].LoadFromDataTable(dtTMP, true);
                    Commons.Modules.MExcel.MFormatExcel(ws1, dtTMP, iDong, this.Name, WidthColumns, true, true, true);

                    iDong = iDong + dtTMP.Rows.Count + 1;
                    iRowBorder = iRowBorder + dtTMP.Rows.Count + 2;
                }

                var border = ws1.Cells[4, 1, iRowBorder + 3, dt1.Columns.Count - 1].Style.Border;
                border.Top.Style = ExcelBorderStyle.Thin;
                border.Left.Style = ExcelBorderStyle.Thin;
                border.Bottom.Style = ExcelBorderStyle.Thin;
                border.Right.Style = ExcelBorderStyle.Thin;

                ws1.Cells[1, 1, iRowBorder + 3, dt1.Columns.Count - 1].Style.Font.Name = "Times New Roman";
                ws1.Cells[1, 1, iRowBorder + 3, dt1.Columns.Count - 1].Style.HorizontalAlignment = OfficeOpenXml.Style.ExcelHorizontalAlignment.Center;
                ws1.Cells[1, 1, iRowBorder + 3, dt1.Columns.Count - 1].Style.VerticalAlignment = OfficeOpenXml.Style.ExcelVerticalAlignment.Center;
        ...
        public int TaoTTChung_TheoDV(Excel.Worksheet MWsheet, int DongBD, int CotBD, int DongKT, int CotKT, float MLeft, float MTop)
        {
            try
            {
                string sSQL = "SELECT TOP 1 * FROM dbo.DON_VI WHERE ID_DV = " + lkDonVi.EditValue;
