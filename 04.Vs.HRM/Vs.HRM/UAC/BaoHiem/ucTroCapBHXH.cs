...
namespace Vs.HRM
{
    public partial class ucTroCapBHXH : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        private void grvDCTroCapBHXH_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            ...
            GridColumn colMucLuongCu = view.Columns["MUC_LUONG_CU"];
            if (e.Column.Name == "colID_CN")
            {
            ...
            if (e.Column.Name == "colNGHI_TU_NGAY" || e.Column.Name == "colNGHI_DEN_NGAY")
            {
                try
                {
                    ...
                    DataTable dtTCBHXH = new DataTable();
                    string strSQL = "SELECT SO_NGAY_NGHI, SO_TIEN_TC FROM dbo.TRO_CAP_BHXH WHERE ID_CN = " + view.GetRowCellValue(e.RowHandle, view.Columns["ID_CN"]) + " AND DOT = " + cboDot.EditValue + " AND THANG = '" + Convert.ToDateTime(cboThang.EditValue).ToString("MM/dd/yyyy") + "' AND NGHI_TU_NGAY = '" + tn.ToString("MM/dd/yyyy") + "'";
                    dtTCBHXH.Load(SqlHelper.ExecuteReader(Commons.IConnections.CNStr, CommandType.Text, strSQL));
                    view.SetRowCellValue(e.RowHandle, view.Columns["SO_NGAY_NGHI"], dtTCBHXH.Rows[0]["SO_NGAY_NGHI"]);
                    view.SetRowCellValue(e.RowHandle, view.Columns["MUC_HUONG_CU"], dtTCBHXH.Rows[0]["SO_TIEN_TC"]);
        ...
        private void grvTroCapBHXH_RowCountChanged(object sender, EventArgs e)
        {
            GridView view = sender as GridView;
            try
            {
