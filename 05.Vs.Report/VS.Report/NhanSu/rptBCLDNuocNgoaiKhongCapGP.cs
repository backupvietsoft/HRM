...
namespace Vs.Report
{
    public partial class rptBCLDNuocNgoaiKhongCapGP : DevExpress.XtraReports.UI.XtraReport
    {
        public rptBCLDNuocNgoaiKhongCapGP(DateTime ngayin)//, DateTime TuNgay, DateTime DenNgay)
        {
            ...
            dtNgu.Load(SqlHelper.ExecuteReader(Commons.IConnections.CNStr, CommandType.Text, "SELECT KEYWORD, CASE " + Commons.Modules.TypeLanguage + " WHEN 0 THEN VIETNAM WHEN 1 THEN ENGLISH ELSE CHINESE END AS NN  FROM LANGUAGES WHERE FORM = N'NgayThangNam' "));
