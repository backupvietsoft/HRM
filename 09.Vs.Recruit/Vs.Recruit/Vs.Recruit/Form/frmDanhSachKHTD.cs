...
{
    public partial class frmDanhSachKHTD : DevExpress.XtraEditors.XtraForm
    {
        public Int64 iID_KHTD = -1;

        public frmDanhSachKHTD()
        {
            InitializeComponent();
        }

        #region even
        private void frmDanhSachKHTD_Load(object sender, EventArgs e)
        {
            LoadData();
        }
        ...
        private void grvDSUV_DoubleClick(object sender, EventArgs e)
        {
            try
            {
                iID_KHTD = Convert.ToInt64(grvDS_KHTD.GetFocusedRowCellValue("ID_TB"));
        ...
        private void LoadData()
        {
            try
            {
                ...
                dt.Load(SqlHelper.ExecuteReader(Commons.IConnections.CNStr, "spGetChonKeHoachTuyenDung", Commons.Modules.UserName, Commons.Modules.TypeLanguage));
                Commons.Modules.ObjSystems.MLoadXtraGrid(grdDS_KHTD, grvDS_KHTD, dt, false, true, false, false, true, this.Name);
                grvDS_KHTD.Columns["ID_TB"].Visible = false;
            }
            catch { }
        }
