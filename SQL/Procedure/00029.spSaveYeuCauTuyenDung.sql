...
ALTER PROCEDURE spSaveYeuCauTuyenDung
    @ID_YCTD  BIGINT = -1,
	@MA_YCTD NVARCHAR(50) = 'TP-2022-07-001',
	@ID_TO BIGINT =11,
	@ID_CN	BIGINT = 30,
	...
	@sBTFile NVARCHAR(250) ='sBTFileadmin'
AS
BEGIN
	BEGIN TRY
	BEGIN TRANSACTION INS_Target
		...
		[ID_LCV] [bigint] NULL,
		[SL_TUYEN] [int] NULL,
		[MUC_LUONG_DK] [nvarchar] (100)  NULL,
		...
		FROM #TEMPT_VT A WHERE A.ID_LCV = @ID_YCTD
		END
		IF LEN(@sBTThayThe) > 0
		BEGIN
		...
		SELECT ID_YCTD,ID_LCV,ID_CN,ID_LCV,NGAY_LV_CUOI,LY_DO_NGHI
		FROM #TEMPT_ThayThe A WHERE A.ID_VTTD IN (SELECT ID_YCTD FROM #TEMPT_VT) AND ID_YCTD = @ID_YCTD
		END
		IF LEN(@sBTFile) > 0
		BEGIN
			...
			SELECT ID_YCTD,A.DUONG_DAN,MO_TA,GHI_CHU  FROM #TEMPT_File A WHERE NOT EXISTS (SELECT * FROM dbo.VI_TRI_FILE B WHERE A.ID_VT_FL = B.ID_VT_FL AND A.ID_YCTD = B.ID_VT_FL)
