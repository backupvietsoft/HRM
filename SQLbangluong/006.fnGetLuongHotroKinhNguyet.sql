...
[LUONG_CA_THANG] [float] ,[KHAU_TRU_TAM_UNG] [float] ,[BHXH_BHTN] [float] ,[BHYT] [float] ,[KHAU_TRU] [float] ,[THUE_TNCN] [float] ,[THUC_LANH] [float] ,[KY_NHAN] [FLOAT])
	...
	--tính Lương cả tháng
	UPDATE #TBLuongThang
	SET LUONG_CA_THANG = ROUND((LUONG_NGAY_CONG + TIEN_THEM_GIO + ISNULL(THUONG_HIEU_SUAT,0) + ISNULL(THUONG_C_HANH,0) + LUONG_PHEP_TIEN + LUONG_LE_TET + ISNULL(THUONG_HTNV,0) + ISNULL(TRO_CAP_CN,0) + ISNULL(TIEN_XANG,0) + ISNULL(THUONG,0) + BU_NCCB),0)
