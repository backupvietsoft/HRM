...
[LUONG_CA_THANG] [float] ,[KHAU_TRU_TAM_UNG] [float] ,[BHXH_BHTN] [float] ,[BHYT] [float] ,[KHAU_TRU] [float] ,[THUE_TNCN] [float] ,[THUC_LANH] [float] ,[KY_NHAN] [FLOAT])
	...
	BHYT = ROUND(LUONG_HDLD * 0.015,0)
		...
		---tính lương lể tết cộng thêm hổ trợ kinh nguyệt(là phái nữ tối thiểu 3 ngày công)
	UPDATE #TBLuongThang
	SET LUONG_LE_TET = ROUND(LUONG_LE_TET + (CASE when DL >= 3 THEN dbo.fnGetLuongHotroKinhNguyet(ID_CN,LUONG_HDLD) ELSE  0 END),0)
	--tính bù nhu cầu cơ bảng
	UPDATE #TBLuongThang
	SET BU_NCCB = ROUND(CASE WHEN	 ISNULL((4700000/26 * DL) - (LUONG_NGAY_CONG + ISNULL(THUONG_HIEU_SUAT,0) + ISNULL(THUONG_C_HANH,0) + LUONG_PHEP_TIEN + LUONG_LE_TET + ISNULL(THUONG_HTNV,0) + ISNULL(TRO_CAP_CN,0) + ISNULL(TIEN_XANG,0) + ISNULL(THUONG,0)),0) > 0 THEN ISNULL((4700000/26 * DL) - (LUONG_NGAY_CONG + ISNULL(THUONG_HIEU_SUAT,0) + ISNULL(THUONG_C_HANH,0) + LUONG_PHEP_TIEN + LUONG_LE_TET + ISNULL(THUONG_HTNV,0) + ISNULL(TRO_CAP_CN,0) + ISNULL(TIEN_XANG,0) + ISNULL(THUONG,0)),0) ELSE 0 END,0)
	--tính Lương cả tháng
	UPDATE #TBLuongThang
	SET LUONG_CA_THANG = ROUND((LUONG_NGAY_CONG + TIEN_THEM_GIO + ISNULL(THUONG_HIEU_SUAT,0) + ISNULL(THUONG_C_HANH,0) + LUONG_PHEP_TIEN + LUONG_LE_TET +
	ISNULL(THUONG_HTNV,0) + ISNULL(TRO_CAP_CN,0) + ISNULL(TIEN_XANG,0) + ISNULL(THUONG,0) + BU_NCCB),0)

	---tính thuế thu nhập cá nhân, cần sửa lại
	UPDATE #TBLuongThang
...
LUONG_LE_TET,THUONG_HTNV,TRO_CAP_CN,TIEN_XANG,THUONG,BU_NCCB,LUONG_CA_THANG,KHAU_TRU_TAM_UNG,BHXH_BHTN,BHYT,KHAU_TRU,THUE_TNCN,THUC_LANH,KY_NHAN)
	...
	GLT,TIEN_THEM_GIO,
	THUONG_HIEU_SUAT,THUONG_C_HANH,LUONG_PHEP_NGAY,
	LUONG_PHEP_TIEN,---lương phép
	NGAY_LE,
	LUONG_LE_TET,---lương lể
	THUONG_HTNV,TRO_CAP_CN,TIEN_XANG,THUONG,
	BU_NCCB,--Bù NCCB
	LUONG_CA_THANG,--Lương cả tháng
	 KHAU_TRU_TAM_UNG,
	 BHXH_BHTN,--BHXH_BHTN
	 BHYT,--BHYT
	 KHAU_TRU,
